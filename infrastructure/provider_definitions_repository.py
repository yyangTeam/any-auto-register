...
from core.db import ProviderDefinitionModel, ProviderSettingModel, engine, normalize_provider_key
...
_BUILTIN_DEFINITIONS: list[dict] = [
    ...
    },
    {
        ...
        "label": "本地邮箱池",
        "description": "导入本地账号池：支持邮箱收码、iCloud/FlySMS 接码地址，以及已注册账号的密码 + MFA 登录",
        ...
        "fields": [
            {
                ...
                "category": "connection",
                "hint": "可选；每行一条账号。接码地址（含 FlySMS）支持邮箱后用3个及以上连字符分隔；另支持心蓝格式、邮箱----密码----MFA、邮箱|密码|2FA。",
            },
            {
                ...
                "type": "textarea",
                "category": "auth",
                "hint": "可选；直接粘贴账号池。接码地址（含 FlySMS）支持 邮箱 + 3个及以上连字符 + URL；已注册账号支持 邮箱----密码----MFA 或 邮箱|密码|2FA。",
            },
            {
...
class ProviderDefinitionsRepository:
    ...
    def get_by_key(self, provider_type: str, provider_key: str) -> ProviderDefinitionModel | None:
        normalized_key = normalize_provider_key(provider_type, provider_key)
        with Session(engine) as session:
            return session.exec(
                select(ProviderDefinitionModel)
                .where(ProviderDefinitionModel.provider_type == provider_type)
                .where(ProviderDefinitionModel.provider_key == normalized_key)
            ).first()
